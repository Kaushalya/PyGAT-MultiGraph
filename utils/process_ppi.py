...
def process_p2p(data_dir):
    print('Loading G...')
    with open(path.join(data_dir, 'ppi-G.json')) as jsonfile:
    ...
    with open(path.join(data_dir, 'ppi-id_map.json')) as jsonfile:
    ...
    features_ = np.load(path.join(data_dir, 'ppi-feats.npy'))
    ...
    class_map = {}
    with open(path.join(data_dir, 'ppi-class_map.json')) as jsonfile:
    ...
    for i in range(1, np.max(list_splits) + 1):
        ...
        if subgraph_.shape[0] < subgraph_nodes or subgraph_.shape[1] < subgraph_nodes:
            ...
            # zero labels for all teh padding nodes
    ...
    val_split = []
    test_split = []
    # TODO move some of the training graphs to test 
    ...
    # TODO check correctness - looks
    adj_sub = torch.FloatTensor(adj_sub)
    feat_sub = torch.FloatTensor(feat_sub)
    labels_sub = torch.LongTensor(labels_sub)
    ...
    data = PpiData(train_adj, val_adj, test_adj, train_feat, val_feat, test_feat, train_labels, val_labels,
                   test_labels, train_nodes, val_nodes, test_nodes, tr_msk, vl_msk, ts_msk)
    return data


if __name__ == "__main__":
    data_dir = '/Users/kaumad/Documents/coding/data/network_data/ppi'
    process_p2p(data_dir)
